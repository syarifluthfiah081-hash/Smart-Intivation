...
export interface LetterTemplate {
  ...
  name: string;
  fields: LetterField[];
  generatePreviewHtml: (variables: Record<string, string>, profile: SchoolProfile) => string;
}
  ...
  // 7. UNDANGAN ORANG TUA / WALI MURID
  {
    ...
    generatePreviewHtml: (vars, _profile) => `
      <div class="font-serif text-black text-[12px] leading-relaxed">
        ...
        <div class="mb-4">
          <p>Kepada Yth.<br/>
          <strong>${vars.penerima || '-'}</strong><br/>
          ${vars.kelas ? `<span>${vars.kelas}</span><br/>` : ''}
          di Tempat</p>
        </div>
          ...
          <table class="w-full border-collapse border border-black">
            ...
            <tbody>
              <tr>
                ...
                <td class="border border-black px-3 py-4 text-left font-semibold">${vars.nama_berkas || '-'}</td>
                <td class="border border-black px-3 py-4 text-center">${vars.jumlah_berkas || '-'}</td>
                <td class="border border-black px-3 py-4 text-left text-[11px]">${vars.keterangan_berkas || '-'}</td>
              </tr>
            </tbody>
          </table>
        </div>
  ...
  // 11. SURAT REKOMENDASI
  {
    ...
    generatePreviewHtml: (vars, profile) => `
      <div class="font-serif text-black text-[12px] leading-relaxed">
        ...
        <div class="mb-4 text-justify">
          <p class="indent-8">Rekomendasi ini diberikan untuk keperluan: <strong>${vars.tujuan_rekomendasi || '-'}</strong>.</p>
